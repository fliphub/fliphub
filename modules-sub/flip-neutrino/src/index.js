...
const requireMiddleware = require('./requirePreset')
...
class Neutrino extends EventEmitter {
  ...
  builder() {
    return new Promise((resolve, reject) => {
      ...
      timer.start('webpack')
      // log.quick(this.config.module.rules)
      // log.quick(this.config.module.entries())
      // log.quick(this.config, config)
      const compiler = webpack(config)
      ...
      compiler.run((err, stats) => {
        ...
        timer.stop('webpack').log('webpack')
        resolve()
      })
    })
  }
...
Neutrino.requireMiddleware = requireMiddleware
