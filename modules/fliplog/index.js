...
class LogChain extends ChainedMapExtendable {
  new() {
    const logger = new LogChain()
    logger.addPreset('error', presetError)
    logger.addPreset('warning', presetWarning)
    logger.addPreset('info', presetInfo)
    logger.addPreset('note', presetNote)
    return logger
  }
  constructor(parent) {
    super(parent)
