...
module.exports = class ContextConfig extends ChainedMap {
  constructor(parent) {
    ...
    this.extend(['root', 'name', 'unified', 'from', 'to', 'flips', 'config', 'inherit', 'presetArgs', 'debug']);
  }
  ...
  mergePresets(presets) {
    ...
    if (this.get('inherit') !== false) {
      this.presets.merge(this.workflow.coreConfig.presets.toConfig());
    }
  ...
  merge(app) {
    ...
    super.merge(data);
    this.mergePresets(presets).mergePresetArgs();
