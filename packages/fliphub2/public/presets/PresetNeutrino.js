...
module.exports = class PresetNeutrino {
  ...
  decorate(context, workflow) {
    ...
    const neutrinos = [];
    Object.keys(used).filter(key => key.includes('neutrino') && key !== 'neutrino' && !key.includes('happypack')).forEach(key => {
      log.tags('neutrino,preset,require').text(`using neutrino preset: ${key}`).color('green.italic').echo();

      // const neutrinoPreset = requirePreset(kebabcase(key)).pop()
      const neutrinoPreset = [require(kebabcase(key))].pop();
      context.presets.add(key, {
